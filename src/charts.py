...
def plot_evolution(ev_df, sym, is_usd, title):
    ...
    fig.update_traces(
        line_color="#00FFAA", fillcolor="rgba(0, 255, 170, 0.2)", hovertemplate="%{y:.2f}"
    )
    fig.update_layout(
        separators=seps, yaxis_title=None, xaxis_title=None, yaxis=dict(tickprefix=f"{sym} ")
    )
    return fig


def plot_allocation(df, names_col, val_col, is_usd, title):
    seps = ".," if is_usd else ", "
    fig = px.pie(df, names=names_col, values=val_col, title=title, hole=0.4, template="plotly_dark")
...
def plot_bar_earnings_horizontal(df, x_label, y_value, sym, is_usd, title):
    ...
    fig = px.bar(
        df_sorted, x=y_value, y=x_label, orientation="h", title=title, template="plotly_dark"
    )
    fig.update_traces(marker_color="#00FFAA", hovertemplate="%{x:.2f}")
    fig.update_layout(
        separators=seps, xaxis_title=None, yaxis_title=None, xaxis=dict(tickprefix=f"{sym} ")
    )
    return fig
