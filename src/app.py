...
def _show_analysis_modal(ticker, analysis, fmt_reg, texts):
    ...
    with _hc2:
        ...
        with st.popover(texts["rec_rationale_title"], width="stretch"):
    ...
    _c1.metric(
        ...
        f"{analysis['yield_pct']:+.1f}%",
        help=texts["metric_help_yield_pct"],
    )
    _c2.metric(
        texts["analysis_breakeven"],
        fmt_reg(analysis["breakeven"]),
        help=texts["metric_help_breakeven"],
    )
    _c3.metric(
        texts["analysis_trailing_stop"],
        fmt_reg(analysis["trailing_stop"]),
        help=texts["metric_help_trailing_stop"],
    )
    _c4.metric(texts["analysis_yoc_label"], f"{_yoc:.1f}%", help=texts["metric_help_yoc"])
    _c5.metric(
        texts["analysis_weight_label"],
        f"{analysis.get('current_weight', 0.0):.1f}%",
    ...
    if analysis.get("price_below_stop"):
        ...
        _rc1.metric(
            texts["analysis_current_return"],
            fmt_reg(analysis.get("current_price", 0)),
            delta=f"{analysis['yield_pct']:+.1f}%",
            delta_color="inverse",
        )
        _rc2.metric(texts["analysis_trailing_stop"], fmt_reg(analysis["trailing_stop"]))
        st.warning(
            texts["risk_exit_alert"].format(
    ...
    if analysis["dca"] and all(_d.get("concentration_risk") for _d in analysis["dca"]):
        ...
        _wc1.metric(
            texts["analysis_post_dca_weight"],
            f"{_worst:.1f}%",
            delta="limite: 10%",
            delta_color="inverse",
        )
    ...
    if rec == "exit":
        ...
        rec == "dca"
        and analysis["dca"]
        and all(_d.get("concentration_risk") for _d in analysis["dca"])
    ):
        _action = texts["action_dca_blocked"]
    elif rec == "dca" and analysis["dca"]:
        _best = next(
            (_d for _d in analysis["dca"] if not _d.get("concentration_risk")), analysis["dca"][0]
        )
...
# Sidebar Controls
# Note: this label is intentionally bilingual because we need the
# selection before we can load `texts`.
...
with st.sidebar.expander(texts["sidebar_market"], expanded=False):
    ...
    auto_refresh = st.toggle(
        texts["auto_refresh_label"], value=True, help=texts["auto_refresh_help"]
    )
...
with st.sidebar.expander(texts["sidebar_import"], expanded=False):
    uploaded_files = st.file_uploader(
        texts["upload_msg"], type=["xlsx"], accept_multiple_files=True
    )
    ...
    if st.session_state.import_stats is not None and not st.session_state.import_stats.empty:
        ...
        if "dedup_removed_main" in st.session_state.import_stats.columns:
            dedup_row = st.session_state.import_stats[
                st.session_state.import_stats["detected"] == "DEDUP"
            ]
            if not dedup_row.empty:
                removed = int(
                    (dedup_row["dedup_removed_main"].fillna(0).iloc[0] or 0)
                    + (dedup_row["dedup_removed_audit"].fillna(0).iloc[0] or 0)
                )
                before = int(dedup_row["rows_total"].fillna(0).iloc[0] or 0)
                after = int(before - (dedup_row["dedup_removed_main"].fillna(0).iloc[0] or 0))
                if removed > 0:
                    st.caption(
                        texts["dedup_summary"].format(removed=removed, before=before, after=after)
                    )
...
if st.session_state.raw_df is not None:
    ...
    res = portfolio_main["ticker"].apply(
        lambda t: (
            (prices.get(t, {}).get("p") or 0) * factor,
            "✅" if prices.get(t, {}).get("live") else "⚠️",
        )
    )
    ...
    portfolio_main["p_atual"] = [
        (
            x[0]
            if x[0] > 0
            else portfolio_main.loc[portfolio_main["ticker"] == t, "avg_price"].values[0] * factor
        )
        for t, x in zip(portfolio_main["ticker"], res)
    ]
    portfolio_main["status"] = [x[1] for x in res]

    for c in ["avg_price", "total_cost", "earnings"]:
        portfolio_main[c] *= factor
    ...
    with tabs[0]:  # Dashboard Global
        ...
        if not cf.empty:
            ...
            ev["cashflow"] *= factor
            c1.plotly_chart(
                charts.plot_evolution(
                    ev.rename(columns={"cashflow": "flow"}), sym, is_usd, texts["chart_evolution"]
                ),
                width="stretch",
        ...
        c2.plotly_chart(
            charts.plot_allocation(
                portfolio_main, "asset_type", "v_mercado", is_usd, texts["chart_allocation"]
            ),
            width="stretch",
            config=PLOTLY_CONFIG,
        )
...
""",
                ...
                with outer_right:
                    st.caption(
                        texts["pagination_showing"].format(start=start + 1, end=end, total=total)
                    )
                    ...
                    if cols[0].button(
                        texts["pagination_prev"], disabled=prev_disabled, key=f"{key_prefix}_prev"
                    ):
                    ...
                    if cols[-2].button(
                        texts["pagination_next"], disabled=next_disabled, key=f"{key_prefix}_next"
                    ):
                    ...
                    cols[-1].selectbox(
                        ...
                        page_size_options,
                        index=(
                            page_size_options.index(page_size)
            ...
            render_paged_df(
                fees_df, ["date", "ticker", "inst", "val", "desc"], key_prefix="audit_fees"
            )
            ...
            render_paged_df(
                transfers_df,
                ["date", "ticker", "inst", "val", "desc"],
                key_prefix="audit_transfers",
            )
            ...
            render_paged_df(
                ignored_df,
                ["date", "ticker", "inst", "val", "desc", "source"],
                key_prefix="audit_ignored",
            )
    ...
    with tabs[ticker_changes_tab_idx]:
        ...
        st.dataframe(pd.DataFrame(remap_rows), width="stretch", hide_index=True)
        ...
        # canonicalize so renamed tickers (see TICKER_REMAP) match portfolio['ticker'].
        _raw_for_first_buy = raw_df[raw_df["type"] == "BUY"].copy()
        _raw_for_first_buy["ticker"] = _raw_for_first_buy["ticker"].apply(
            lambda t: utils.TICKER_REMAP[t]["new"] if t in utils.TICKER_REMAP else t
        )
        _first_buy = _raw_for_first_buy.groupby("ticker")["date"].min().apply(pd.Timestamp)
        ...
        for t in sorted(_portfolio_tickers):
            ...
            for ev in split_history[t]:
                ...
                ev_type = (
                    texts["ticker_changes_corp_type_reverse"]
                    if is_reverse
                    else texts["ticker_changes_corp_type_split"]
                )
                if is_reverse:
        ...
        if corp_rows:
            ...
            st.dataframe(corp_df, width="stretch", hide_index=True)
        else:
        ...
        _no_live = [
            t for t in portfolio_main["ticker"].unique() if not prices.get(t, {}).get("live")
        ]
        ...
        if _no_live:
            ...
            st.dataframe(pd.DataFrame(possibly_disc_rows), width="stretch", hide_index=True)
        else:
