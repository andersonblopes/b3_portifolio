...
if st.session_state.raw_df is not None:
    ...
    with tabs[0]:  # Dashboard Geral
        ...
        if not df_ev.empty:
            ...
            ev['flow'] *= factor
            c1.plotly_chart(charts.plot_evolution(ev, sym, is_usd, texts['chart_evolution']), use_container_width=True)
        c2.plotly_chart(
        ...
        if has_earnings:
            st.divider()
            c3, c4 = st.columns(2)
            df_m = raw_df[raw_df['type'] == 'EARNINGS'].copy()
            df_m['month_year'] = df_m['date'].dt.strftime('%Y-%m')
            res_m = df_m.groupby('month_year')['val'].sum().reset_index().sort_values('month_year')
            res_m['val'] *= factor
            c3.plotly_chart(charts.plot_earnings_evolution(res_m, sym, is_usd, texts['chart_earn_monthly']),
                            use_container_width=True)
    ...
    if has_earnings:
        with tabs[2]:
            ...
            with r1_c1:
                ...
                st.plotly_chart(charts.plot_pie_earnings(e_type, 'sub_type', 'val', is_usd, texts['chart_earn_type']),
                                use_container_width=True)
            with r1_c2:
                earn_raw['at_type'] = earn_raw['ticker'].apply(utils.detect_asset_type)
                e_asset = earn_raw.groupby('at_type')['val'].sum().reset_index()
                st.plotly_chart(
                    charts.plot_pie_earnings(e_asset, 'at_type', 'val', is_usd, texts['chart_earn_asset_type']),
                    use_container_width=True)
